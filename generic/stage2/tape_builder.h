...
namespace stage2 {
    ...
    private:
        ...
        simdjson_really_inline void start_container(json_iterator& iter, internal::tape_type start) noexcept;
    ...
    simdjson_warn_unused simdjson_really_inline error_code tape_builder::visit_string(json_iterator& iter, const uint8_t* value, bool key) noexcept {
        ...
        //Sstd::cout << " chk " << start << " || " << (size_t)(dst-start) << "\n";
        tape.append_str(std::string_view((char*)start, (size_t)(dst - start)), key);
        return SUCCESS;
    }
